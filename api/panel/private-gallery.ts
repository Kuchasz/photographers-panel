...
export const getGalleriesList = () => f.get<GalleryDto[]>(getGalleriesListRoute);
...
export const checkPasswordIsUnique = (galleryId?: number) => (password: string) =>
    f.get<boolean>(
        checkPasswordIsUniqueRoute.replace(':password', password).replace(':galleryId?', galleryId?.toString() ?? '')
    );
checkPasswordIsUnique.route = checkPasswordIsUniqueRoute;

const createGalleryRoute = '/api/panel/create-gallery';
export const createGallery = (gallery: GalleryEditDto) => f.post<CreateGalleryResult>(createGalleryRoute, gallery);
...
export const deleteGallery = (id: number) => f.post<DeleteGalleryResult>(deleteGalleryRoute, { id });
...
export const notifySubscribers = (selectedGallery: number) =>
    f.post<NotifySubscribersResult>(notifySubscribersRoute, {
        id: selectedGallery,
    });
