    ...
    new Promise<{
        ...
        fetch(`${path}folders.xml`)
            ...
            .then((foldersXmlString) =>
                parseString(foldersXmlString, (_, _directories: { item: { item: any[] } }) => {
                    ...
                    directoriesToFetch.forEach((item, index) => {
                        fetch(`${path}${item.variables}`)
                            ...
                            .then((photosXmlString) => {
                                parseString(photosXmlString, (_, photos: { gallery: { image: any[] } }) => {
                                    if (photos.gallery) {
                                        const _images = photos.gallery.image.map((img: any) => img.$);
